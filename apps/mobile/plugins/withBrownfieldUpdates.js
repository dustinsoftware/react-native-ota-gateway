...
// RCTBundleURLProvider (the DEBUG Metro bundle-URL override) lives in
...
const INITIALIZE_UPDATES = `
  ...
  /// points at the local Metro dev server (Metro mode -- local hot reload); Release
...
#if DEBUG
    // Metro mode (local hot reload): the JS bundle comes from the Metro dev
    // server. A Debug build leaves expo-updates in its disabled state
    // (EXAppDefines.APP_DEBUG -> a disabled controller), so nothing else offers a
    // bundle URL -- THIS override is what points the runtime at Metro. Resolve
...
#else
    // Shipping mode (release): every brownfield surface must boot the bundle
