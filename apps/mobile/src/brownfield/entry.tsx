...
// Brownfield entry -- native host passes initialUrl via initialProperties.
// The per-mount context identity keeps expo-router from restoring the
// previous native screen's navigation state on Android -- see
// freshRouteContext. The useState is load-bearing: inlining
// freshRouteContext(ctx) in render would mint a new identity every render
// and reset the router's state mid-session.
function BrownfieldApp(props: { initialUrl?: string }) {
  const [context] = useState(() => freshRouteContext(ctx));
