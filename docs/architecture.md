...
either from Metro (**Metro mode**) or from the self-hosted gateway (**Shipping
mode**). The gateway is the app's *own* Express server, run as two instances over
one export so a single build can demonstrate a dev/prod environment split.
...
flowchart TD
    ...
    METRO -->|"Metro mode · Fast Refresh"| IOS
    METRO -->|"Metro mode · Fast Refresh"| AND
    GW -->|"Shipping mode · OTA manifest + bundle"| IOS
    GW -->|"Shipping mode · OTA manifest + bundle"| AND
```
...
whom](#who-talks-to-whom), and the [runtime modes and artifact
source](#runtime-modes-and-artifact-source).
...
|- docker-compose.yml         # standalone gateway containers -- REQUIRED serving for Shipping mode (dev :3000, prod :3001)
...
```
   ...
   +---------+---------+                                    +----------+--------+
             |                                                         |
             |  Shipping mode: embedded/OTA JS bundle    Metro mode: Metro (:8081)
             |                                                         |
   +---------v-----------------------------+   +-------------------v---------------+
...
## Runtime modes and artifact source

Two **independent** questions decide how the brownfield RN runs in a host. They
used to be collapsed into one "Mode A / Mode B" list, which could not express the
legal combinations:

1. **Runtime mode -- where does the JS come from?** `Metro mode` (the local dev
   server, Fast Refresh) or `Shipping mode` (the OTA manifest / the bundle
   embedded in the artifact -- what actually ships).
2. **Artifact source -- where did the native artifact come from?** `prebuilt`
   (downloaded from a release; **no React Native compile**) or a `source build`
   (compiled from this repo -- `package-ios.sh`, `brownfield publish:android`).

They compose: "Metro mode on the prebuilt Debug asset" is the everyday JS loop,
"Shipping mode on a source build" is how packaging is validated before a cut.

| | Metro mode | Shipping mode |
| --- | --- | --- |
...
| Artifact source | the release's prebuilt `-debug` asset, or a Debug source build | the release's prebuilt Release asset, or a Release source build |

### Per-platform mechanism (they differ)

| Platform | Metro mode mechanism | Shipping mode mechanism |
| --- | --- | --- |
| **iOS** | **Debug-built framework** -- `pnpm install:ios --debug` downloads the prebuilt Debug asset, `--local --debug` installs the `build-debug/` mirror, or `./scripts/package-ios.sh --configuration Debug` builds one. A `bundleURLOverride` injected under `#if DEBUG` points the runtime at Metro; a Debug build also leaves expo-updates in its disabled state, so nothing competes for the bundle URL. **There is no runtime Metro toggle on iOS**: a Release framework's enabled expo-updates owns the bundle URL and always wins, so you switch modes by swapping frameworks and rebuilding the host. | Release-built framework (`pnpm install:ios`, or `./scripts/package-ios.sh --configuration Release`); expo-updates enabled; JS from the OTA manifest / embedded bundle. |
| **Android** | **Runtime toggle** -- a "Use Metro dev server" pref in native Host Settings (opened from Developer). When on, the host inits RN via a dev host manager (`useDevSupport = true`); no artifact rebuild or reinstall needed. | Release AAR (or the toggle off); JS from OTA / embedded bundle. |
...
in a Debug build (compile-time), so the runtime mode is a property of the
installed framework -- which is why releases publish both variants. Android's
