---
name: verify-android
description: Verify the Android brownfield host in both runtime modes -- Metro mode (local hot reload, JS from Metro via the runtime toggle) and Shipping mode (release artifact, JS from OTA/embedded). Use when asked to verify, test, or smoke-check the Android app/host, run the Android OTA flow, or switch the Android host between Metro mode and Shipping mode.
---

# Verify Android (Metro mode / Shipping mode)
...
The toggle works against the AAR you already have, so **artifact source** (a
published AAR vs a local `brownfield publish:android`) is an independent axis
here -- a source build is for native changes or packaging validation, never
just to get hot reload. See `docs/architecture.md`.
...
> dev server" toggle** selects Metro mode vs Shipping mode (JS source); the
> **environment radio** selects dev vs prod gateway (`:3000` vs `:3001`) and is a Shipping-mode
...
7. **Port reversing (emulator too, not just physical devices):**
   ...
   ```
   Without `:8081` in Metro mode the app's own `localhost` requests die even if the
...
> Only an uninstall clears it back to the default (off = Shipping mode).
>
> This holds only *within* one `applicationId`. If a previous
> `dev.otagateway.host` build is installed, uninstall it once
> (`adb uninstall dev.otagateway.host`) -- the app id changed, so that build is a
> separate app with its own prefs and its own launcher icon, and the new one
...
2. **Pass criteria:** the embedded bundle loads; Developer shows the current
   `OTA marker` and an update id; the RN tab bar is hidden. Host packaging is
   intact (nothing else checks this -- CI never builds the host):
   `aapt2 dump badging app/build/outputs/apk/debug/app-debug.apk` reports
   `package: name='com.regalcinemas.reactnativetest'` and
   `icon='res/mipmap-anydpi-v26/ic_launcher.xml'`, and the launcher icon is not
   the stock Android robot. (The placeholder adaptive icon renders as a white
   circle by design -- see docs/brownfield.md.) The Maestro suite
...
## Metro mode -- local hot reload (no rebuild)
...
**Return to Shipping mode:** turn the toggle off and relaunch. (Note: Metro mode
leaves `HostEnvironment.current` unset -- the native env radio is a Shipping-mode
concern; Metro owns the bundle in Metro mode.)
...
- `adb install -r` keeps the toggle state -- an unexpected runtime mode can persist
