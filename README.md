...
delivered into a brownfield host, in both Metro mode (hot reload) and Shipping
mode (release/OTA), on both platforms.
...
gateway seam. For Shipping-mode (release artifact) testing the two instances run as
...
The brownfield host's Developer tab, running the self-hosted OTA bundle (Shipping mode)
...
| Doc | Covers |
| --- | --- |
| [docs/architecture.md](./docs/architecture.md) | Monorepo layout, the dual-target model, who talks to whom, the runtime-mode matrix per platform. |
...
| [docs/brownfield.md](./docs/brownfield.md) | Packaging pipeline, the two config plugins, what is stock upstream vs ours, the full iOS + Android host-integration recipe, gotchas. |
...
| [docs/development-workflow.md](./docs/development-workflow.md) | Prerequisites, runtime-mode steps per platform, the runbook, verification. |
...
| Host app id (iOS bundle id / Android `applicationId`) | `com.regalcinemas.reactnativetest` |
...
| **Runtime mode** | The first axis: where a host gets its JS -- **Metro mode** or **Shipping mode**. |
| **Metro mode** | Runtime mode where the host loads JS from Metro (`:8081`) with Fast Refresh. For iterating on RN/JS. |
| **Shipping mode** | Runtime mode where the host loads JS from the OTA manifest / embedded bundle (no Metro). The shippable path. |
| **Artifact source** | The second axis: whether the native artifact was **prebuilt** (downloaded from a release -- no React Native compile) or a **source build** (compiled here). Either source can run either runtime mode. |
...
3  docker compose up --build -d                    # gateway containers (Shipping-mode serving) -- dev :3000, prod :3001
...
14 pnpm install:ios --local --debug -> rebuild host (own -derivedDataPath) -> Metro mode
...
brownfield host integration in both Metro mode and Shipping mode, on the iOS simulator and
