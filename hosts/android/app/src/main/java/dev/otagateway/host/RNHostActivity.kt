...
/**
 * Launcher and host shell. Mirrors the product hosts' lifecycle: one shared,
 * already-initialized brownfield runtime (booted in [OtaHostApplication])
 * with at most ONE mounted [ReactNativeFragment] at a time. The shell is a
 * toolbar, a single content container, and a [BottomNavigationView] with the
 * Developer / Sky / Spinner / More tabs. The first three mount an RN surface;
 * More is NATIVE -- a Test menu whose rows PUSH dedicated screens on the back
 * stack ([RNScreenActivity] for the RN Test 1/2, [NativeTestActivity] for the
 * native Test 3), so while More is selected the only live RN surface is a
 * pushed one.
 *
